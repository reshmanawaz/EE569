...
void read_raw_file(char* image_name, vector<vector<vector<unsigned char> > > &original_image){
    ...
    //rgb - similar to grey but now its 3 not 1
    for(int i = 0; i < original_height; i++){
        for(int j = 0; j < original_width; j++){
            fread(&original_image[i][j][0] , sizeof(unsigned char), 1, file_name);
            fread(&original_image[i][j][1] , sizeof(unsigned char), 1, file_name);
            fread(&original_image[i][j][2] , sizeof(unsigned char), 1, file_name);
...
void write_raw_file(char* image_name, vector<vector<vector<unsigned char> > > &output_data){
    ...
    for(int i = 0; i < original_height; i++){
        for(int j = 0; j < original_width; j++){
            ...
            fwrite(&output_data[i][j][1], sizeof(unsigned char), 1, file_name);
            fwrite(&output_data[i][j][2], sizeof(unsigned char), 1, file_name);
        }
    }
    fclose(file_name);
}

//function has been modified to take care of the 3 rgbs rather than hard coding it to 1
...
void apply_bilateral_filter(vector<vector<vector<unsigned char> > > &mirror, vector<vector<vector<unsigned char> > > &output_data) {
    const double sigma_c_value = 20.0;
    const double sigma_s_value = 10.0;
    const int size = 5;
    ...
    for (int row = extend_size; row < (mirror.size() - extend_size); row++) {
        for (int coloumn = extend_size; coloumn < (mirror[0].size() - extend_size); coloumn++) {
            for (int channel = 0; channel < num_channels; channel++) {
                double numerator = 0.0;
                double denominator = 0.0;

                for (int i = row - extend_size; i <= row + extend_size; i++) {
                    for (int j = coloumn - extend_size; j <= coloumn + extend_size; j++) {
                        const double spatial_term = exp(-((row - i) * (row - i) + (coloumn - j) * (coloumn - j)) / (2 * sigma_c_value * sigma_c_value));
                        const double intensity_term = exp(-(mirror[row][coloumn][channel] - mirror[i][j][channel]) * (mirror[row][coloumn][channel] - mirror[i][j][channel]) / (2 * sigma_s_value * sigma_s_value));
                ...
                output_data[row - extend_size][coloumn - extend_size][channel] = pixel_val_round(numerator / denominator);
            }
        }
    }
}
...
{   
    ...
    vector<vector<vector<unsigned char> > > bilateral_output(original_height+4, vector<vector<unsigned char> > (original_width+4, vector<unsigned char> (rgb)));
    ...
    for(int i=0; i<k; i++)
    {
            mirror_original_image(bilateral_output, 5, output_data);
            apply_bilateral_filter(bilateral_output, output_data);
    }
